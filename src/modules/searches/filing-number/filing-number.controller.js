...
async function caseData(req, res) {
  const { sessionId, filyear, stateCode, distCode, estCode } = req.body;
  ...
  const captchaCode = firstDefined(req.body.captchaCode, req.body.file_captcha_code);
  const courtComplexCode = firstDefined(
  ...
  if (!sessionId || !filingNo || !filyear || !captchaCode) {
  ...
  try {
    ...
    if (String(resolvedState) === "3" && !caseType) {
      return res.status(400).json({
        success: false,
        status: 0,
        error: MESSAGES.MISSING_CASE_TYPE_FOR_STATE,
      });
    }

    let layer1Data;
    try {
      layer1Data = await postWithRetry(session, "casestatus/submitFillingNo", {
        case_type: caseType,
        filing_no: filingNo,
        filyear,
        file_captcha_code: captchaCode,
    ...
    const normalizedSearch = normalizeFilingNumberSearchResponse(layer1Data);
    ...
    if (!layer1Html) {
      return res.status(422).json({
        success: false,
        status: 0,
        error: MESSAGES.EMPTY_FILING_DATA_RESPONSE,
        nextCaptcha,
      });
    }
    ...
    for (const currentCase of cases) {
      ...
      try {
        const detailHtml = await fetchViewHistory(session, {
          ...
          searchFlag: viewDetails.searchFlag || "CSfilingNumber",
          ...
          searchBy: viewDetails.searchBy || "CSfilingNumber",
        });
    ...
    const payload = buildFilingNumberSearchSuccessResponse({
      normalizedSearch,
      layer1Html,
      cases: enrichedCases,
      metadata,
